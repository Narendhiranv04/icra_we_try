...
def test_lid_occupancy_predicates():
    ...
    is_occ_clear, culprits_clear = check_lid_occupancy(model_clear, data_clear)
    assert not is_occ_clear, "Lid clear scene was falsely marked as occupied!"
    assert len(culprits_clear) == 0

    # 2. Lid occupied scene
    blocker_objs = [{"name": "blocker1", "type": "coffee_can", "pos": [0.52, 0.18, 0.82]}]
    model_occ, data_occ = builder.create_environment(objects_to_spawn=blocker_objs, settle_steps=50)
    is_occ, culprits = check_lid_occupancy(model_occ, data_occ, blocker_names=["blocker1"])
