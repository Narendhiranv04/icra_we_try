...
def test_target_occupancy_predicates():
    ...
    is_occ_clear, culprits_clear, meas_clear = check_target_occupancy(model_clear, data_clear)
    assert not is_occ_clear, "Target clear scene was falsely marked as occupied!"
    assert len(culprits_clear) == 0

    # 2. Target occupied scene
    occupant_objs = [{"name": "occupant", "type": "sugar_box", "pos": [-0.10, -0.20, 0.65]}]
    model_occ, data_occ = builder.create_environment(objects_to_spawn=occupant_objs, settle_steps=100)
    is_occ, culprits, meas_occ = check_target_occupancy(model_occ, data_occ, candidate_objects=["occupant"])
