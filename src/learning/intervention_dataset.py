...
- Model inputs are strictly pre-intervention observations.
- Geometry is task-relative (meters).
- NONE interventions are neutrally tensorized with zero-padding.
- Samplers preserve complete candidate sets per scene for ranking loss.
"""
...
class InterventionLearningDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        # 2. Pre-Scene Visual Features (Global & Patch)
        ...
        # 3. Intervention Candidate Tensors
        ...
        if operator_str == "NONE":
            ...
            candidate_present = torch.tensor(0.0, dtype=torch.float32)
            ...
            # Extract object name from crop path or privileged metadata
            p_meta = rec.get("privileged_metadata", {})
            obj_name = p_meta.get("candidate_object_name")
            if not obj_name:
                obj_name = Path(crop_path).stem.replace("object_", "")

            crop_feat_file = self.features_dir / "crops" / f"crop_{scene_id}_{obj_name}_features.pt"
            ...
            candidate_present = torch.tensor(1.0, dtype=torch.float32)
        else:
            raise ValueError(f"Unsupported intervention operator: {operator_str}")

        demo_present = torch.tensor(0.0, dtype=torch.float32)
...
def collate_intervention_group(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    collated_model_inputs = {
        ...
        "candidate_present": torch.stack([m["candidate_present"] for m in m_in_list]),
        "demo_present": torch.stack([m["demo_present"] for m in m_in_list]),
    }
